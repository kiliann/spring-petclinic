 ...
 */
/*
 * @WebMvcTest(value = PetController.class, includeFilters = @ComponentScan.Filter(value =
 * PetTypeFormatter.class, type = FilterType.ASSIGNABLE_TYPE)) class PetControllerTests {
 *
 ...
 * cat.setName("hamster");
 * given(this.owners.findPetTypes()).willReturn(Lists.newArrayList(cat)); Owner owner =
 * new Owner(); Pet pet = new Pet(); owner.addPet(pet); pet.setId(TEST_PET_ID);
 ...
 * .andExpect(view().name("pets/createOrUpdatePetForm")).andExpect(model().attributeExists
 * ("pet")); }
 *
 ...
 * "Betty") .param("type", "hamster").param("birthDate",
 ...
 * .andExpect(model().attributeHasErrors("pet")).andExpect(model().attributeHasFieldErrors
 * ("pet", "type")) .andExpect(model().attributeHasFieldErrorCode("pet", "type",
 ...
 * TEST_PET_ID).param("name", "Betty") .param("type", "hamster").param("birthDate",
 * "2015-02-12")).andExpect(status().is3xxRedirection())
 * .andExpect(view().name("redirect:/owners/{ownerId}")); }
 *
 * @Test void testProcessUpdateFormHasErrors() throws Exception {
 * mockMvc.perform(post("/owners/{ownerId}/pets/{petId}/edit", TEST_OWNER_ID,
 * TEST_PET_ID).param("name", "Betty") .param("birthDate",
 * "2015/02/12")).andExpect(model().attributeHasNoErrors("owner"))
